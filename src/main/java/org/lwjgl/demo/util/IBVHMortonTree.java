...
public class IBVHMortonTree<T extends MortonBoundable<T>> {
    ...
    private static <T extends MortonBoundable<T>> IBVHMortonTree<T> build(IBVHMortonTree<T> parent,
        ...
        tree.minX = Math.min(tree.left.minX, tree.right.minX);
        tree.minY = Math.min(tree.left.minY, tree.right.minY);
        tree.minZ = Math.min(tree.left.minZ, tree.right.minZ);
        tree.maxX = Math.max(tree.left.maxX, tree.right.maxX);
        tree.maxY = Math.max(tree.left.maxY, tree.right.maxY);
        tree.maxZ = Math.max(tree.left.maxZ, tree.right.maxZ);
